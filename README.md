...
- **Role-based access** — three permission levels: group admin, event creator, and regular member
...
| Permission | Group Admin | Event Creator | Member |
|---|---|---|---|
| Invite members / view join link | ✓ | | |
...
4. Update `ALLOWED_ORIGINS` to your app URL after the first deploy

The build command compiles the React frontend, and FastAPI serves it as static files alongside the API.
...
| `DB_PATH` | Prod | Path to SQLite file (defaults to project directory) |
